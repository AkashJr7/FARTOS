...
def cvloop(run_event):
    ...
    # Close the Tkinter window and redirect to checkout.html
    root.destroy()
    import webbrowser
    # Pass the CART list to the Flask backend
    cart_items = ",".join(CART)  # Convert the list to a comma-separated string
    webbrowser.open(f"http://127.0.0.1:5000/checkOut?cart_items={cart_items}")
'''
def cvloop(run_event):
    global ctr_mid
    global SPRITES
...
#                apply_sprite(image, IMAGES[5][ACTIVE_IMAGES[5]],w,x,y, incl, ontop = True)
    ...
    root.destroy()
    import webbrowser
    webbrowser.open("http://127.0.0.1:5000/checkOut")
'''
def cvloop(run_event):
    global ctr_mid
    global SPRITES
...
def apply_sprite(image, path2sprite, w, x, y, angle, ontop=True):
    sprite = cv2.imread(path2sprite, -1)
    if sprite is None:
        print(f"Error: Unable to load sprite at {path2sprite}")
        return image
    sprite = rotate_bound(sprite, angle)
    (sprite, y_final) = adjust_sprite2head(sprite, w, y, ontop)
    image = draw_sprite(image, sprite, x, y_final)
...
def cvloop(run_event):
    ...
    while run_event.is_set():
        ...
        for face in faces:
            ...
            if SPRITES[3]:
                image = apply_sprite(image, IMAGES[3][ACTIVE_IMAGES[3]], w + 45, x - 20, y + 20, incl, ontop=True)
