...
predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")  # Download and place in the project

def overlay_goggles(frame, goggles_path):
    goggles = cv2.imread(goggles_path, cv2.IMREAD_UNCHANGED)
    ...
    for face in faces:
        ...
        goggles_width = int(1.5 * (right_eye[0] - left_eye[0]))
        goggles_height = int(goggles.shape[0] * (goggles_width / goggles.shape[1]))
        
        x_offset = left_eye[0] - int(0.25 * goggles_width)
        y_offset = left_eye[1] - int(0.5 * goggles_height)
        
        goggles = cv2.resize(goggles, (goggles_width, goggles_height))
        
        for c in range(3):
            frame[y_offset:y_offset+goggles_height, x_offset:x_offset+goggles_width, c] = (
                goggles[:, :, c] * (goggles[:, :, 3] / 255.0) + 
                frame[y_offset:y_offset+goggles_height, x_offset:x_offset+goggles_width, c] * (1.0 - goggles[:, :, 3] / 255.0)
            )
